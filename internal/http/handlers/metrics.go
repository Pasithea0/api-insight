...
func Export(db *gorm.DB) fasthttp.RequestHandler {
	return func(ctx *fasthttp.RequestCtx) {
		user, ok := MustUser(ctx)
		if !ok {
			return
		...
		if source == "" {
			errResponse(ctx, fasthttp.StatusBadRequest, "missing source")
			return
		}

		ctx.SetContentType("text/csv")
		ctx.Response.Header.Set("Content-Disposition", fmt.Sprintf("attachment; filename=\"%s_export_%s.csv\"", source, time.Now().Format("2006-01-02")))
		...
		case "recent", "all-events", "search-events":
			...
			if source == "search-events" {
				...
				pattern := string(ctx.QueryArgs().Peek("pattern"))
				matchType := string(ctx.QueryArgs().Peek("type"))
				if field != "" && pattern != "" {
					var sqlPattern string
					switch matchType {
					case "ends_with":
